(ns camelot.report.module.builtin.columns.media-capture-time-breakdown-test
  (:require [camelot.report.module.builtin.columns.media-capture-time-breakdown :as sut]
...
(facts "Media capture timestamp breakdown"
  (fact "Calculates breakdown for date"
  ...
  (fact "Calculates breakdown for time"
    (let [data [{:trap-station-id 1
                 :taxonomy-id 1
                 :media-capture-timestamp (t/date-time 2015 1 10 5 0 0)}
                {:trap-station-id 1
