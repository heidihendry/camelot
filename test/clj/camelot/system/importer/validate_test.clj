(ns camelot.system.importer.validate-test
  (:require [camelot.system.importer.validate :as sut]
  ...
  (testing "list-record-problems"
    ...
    (testing "should run default tests if no tests provided"
      (is (= (list-record-problems
             ...
             [{:result :fail
               :test :camelot.system.importer.validate/session-dates
               :row 2}])))))
      ...
      (with-redefs [camelot.util.file/length (fn [x] 1000)
        ...
        (let [data [{:trap-station-session-start-date (t/date-time 2016 1 1)
          ...
          (is (= (sort (map #(:test %) result))
                 [:camelot.system.importer.validate/camera-overlaps
                  :camelot.system.importer.validate/camera-overlaps
                  :camelot.system.importer.validate/session-dates])))))))
